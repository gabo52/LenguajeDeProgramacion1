...
ofstream & operator <<(ofstream &arch,Conductores &conduc){
    ...
    arch<<setw(8)<<left<<"No."<<setw(20)<<left<<"Fecha"<<setw(15)<<left<<"Placa"
            <<setw(18)<<left<<"Infraccion"<<setw(15)<<left<<"Gravedad"
            <<left<<"Multa"<<endl;
    ...
    for(int i = 0;i<conduc.GetNumFaltas();i++){
        ...
        arch<<"  "<<setw(15)<<left<<placa
            <<setw(18)<<left<<aux[i].GetCodInf()<<setw(15)<<left<<gravedad
            <<left<<aux[i].GetMulta()<<endl;
    }
    imprimeLinea(arch,MAXCANTLINEA,'-');
    arch<<setw(50)<<right<<"Cantidad"<<setw(18)<<right<<"Total"<<endl;
    arch<<setw(40)<<right<<"Total de Infracciones:"<<setw(8)<<conduc.GetNumFaltas()
            <<setw(8)<<right<<(++conduc)<<endl;
}